...
class mobileConnect {
   ...
   private $oidc_userinfo_ep = "https://india.mconnect.wso2telco.com/oauth2/userinfo?schema=openid";
   ...
   public function mobileconnectAuthenticated($argv1 = "") {
   ...
   public function mobileconnectAuthenticate($loginhnt, $scope, $type) {
      $this->oidc_state = "state".substr(base64_encode(md5( mt_rand() )), 0, 13);
      $this->oidc_nonce = "nonce".substr(base64_encode(md5( mt_rand() )), 0, 13);
      ...
      if ($loginhnt !=0) {
         $url .= "&login_hint=ENCR_MSISDN:" . $loginhnt;
         if($type == 2)
         {
            $this->mnv_msisdn =  $loginhnt;
         }
   ...
   // perform Discovery GET request with selected mcc and mnc
   public function PerformGetDiscoverySelectedMCCMNC($selectmcc, $selectmnc) {
      $url = $this->discovery_ep . "?Redirect_URL=" . $this->redirect_uri . "&Selected-MCC=" . $selectmcc . "&Selected-MNC=" . $selectmnc;
   ...
   public function curl_post($url, $auth, $data) {
   ...
   public function discoveryCallback($argv1) {
   ...
   private function mobileconnectComplete($argv1,$argv2) {
      ...
      $values = json_decode($this->userinfo_payload);
      if($_SESSION['type'] == 1)
      {
        echo "<div align='center'>You have connected through WiFi network</div><br />";
      }
      else if($_SESSION['type'] == 0){
        echo "<div align='center'>You have connected through Mobile Network</div><br />";
      }
      echo "<div align='center'><strong> <h1>Successfully Completed the entire Flow!</h1> <br /> The values are: <br />PCR = " . $values->sub . "<br/>MSISDN = " . $values->msisdn . "</strong></div>";
   }
